    ...
    votingPhaseCustomersBySearch: async (parent, { gender, searchTerm, limit = 20, offset = 0 }) => {
      return await sequelize
        .query(
          `
            select
              votes_table.customer_id as id, full_name, email, slug, votes, votes_rank, city,
              case when profile_pic is null then null else (
                case
                  when profile_pic = 'pic1' then pic1
                  when profile_pic = 'pic2' then pic2
    ...
    top500LeaderboardBySearch: async (parent, { gender, searchTerm, limit = 20, offset = 0 }) => {
      return await sequelize
        .query(
          `
            select
              top500_table.customer_id as id, full_name, email, slug, votes, score, city,
              case when profile_pic is null then null else (
                case
                  when profile_pic = 'pic1' then pic1
                  when profile_pic = 'pic2' then pic2
  ...
  Mutation: {
    activateOnlineVoting: combineResolvers(isAuthenticated, async (parent, { status }, { models, me }) => {
      ...
      if (status) {
        ...
        // After male/female initialization
const approvedHairstylists = await sequelize.query(
  `
    SELECT customer_id, full_name, email, phone
    FROM profiles
    LEFT OUTER JOIN customers ON customers.id = customer_id
    WHERE customers.active IS TRUE
      AND gender = 'h'
      AND final_status = 'approved'
  `,
  { raw: true, type: sequelize.QueryTypes.SELECT }
...
for await (const hairstylist of approvedHairstylists) {
  const found = await models.OnlineVote.findOne({
    where: { customer_id: Number(hairstylist.customer_id) }
  });
  if (found) {
    await found.update({ active: true, updated_by: Number(me.id) });
  } else {
    await models.OnlineVote.create({
      ...
      created_by: Number(me.id),
        ...
        const approvedForVoting = [...approvedFemales, ...approvedMales, ...approvedHairstylists];
    ...
    finaliseTop500: combineResolvers(isAuthenticated, async (parent, { status }, { models, me }) => {
      ...
      if (status) {
        ...
        // After male/female sections
const top500Hairstylists = await sequelize.query(
  `
    SELECT customer_id, email, phone, full_name
    FROM online_votes
    LEFT OUTER JOIN customers ON customers.id = customer_id
    WHERE online_votes.active IS TRUE
      AND online_votes.gender = 'h'
      AND customers.active IS TRUE
    ORDER BY votes DESC
    LIMIT 500
  `,
...
for await (const hairstylist of top500Hairstylists) {
  const found = await models.Top500.findOne({
    where: { customer_id: Number(hairstylist.customer_id) }
  });
  if (found) {
    await found.update({ active: true, updated_by: Number(me.id) });
...
// Update finalTop500 array
const finalTop500 = [...top500Females, ...top500Males, ...top500Hairstylists];


        // const finalTop500 = [...top500Females, ...top500Males];
        const finalTop500IDs = finalTop500.map(({ customer_id }) => Number(customer_id));

        let disqualified = [];
        if (finalTop500IDs && finalTop500IDs.length)
          disqualified = await sequelize.query(
            `
              ...
              where
                online_votes.active is true
                and online_votes.customer_id not in (${finalTop500IDs})
                and customers.active is true
              order by votes desc
            `,
            { raw: true, type: sequelize.QueryTypes.SELECT }
        ...
        // Send e-mail & SMS to Top-500 winners
        schedule.scheduleJob(`sendMail-top500Winners`, new Date(Date.now() + 2000), async () => {
          for await (const contestant of finalTop500) {
            const sendEmailArgs = {
              templateName: 'top500Winners',
              templateData: { name: contestant.full_name.split(' ')[0] },
              toAddress: contestant.email.toLowerCase(),
              subject: 'Congratulations you are in Top 500 | OMG – Face Of The Year 2025',
            };
            sendEmail(sendEmailArgs).then(
              () => console.log(`Top-500 mail successfully sent to: ${contestant.email.toLowerCase()}.`),
              (err) => {
                console.log(`Error while sending top-500 mail to: ${contestant.email.toLowerCase()}.`);
                console.dir(err.message);
              }
            );

            const smsText =
              'Dear Candidate,%0aCongratulations! You have been shortlisted in our Top 500. Get ready for the next round.%0aRegards,%0aOMG - Face Of The Year 2025';
            await axios
              .post(
              ...
              .then((res) => console.log(`RESPONSE RECEIVED FOR TRANSACTIONAL SMS TO ${contestant.phone} : `, res.data))
              .catch((err) => console.log(`ERROR FOR TRANSACTIONAL SMS TO ${contestant.phone} : `, err));
          }
        });

        // Send e-mail to disqualified from Top-500
        if (disqualified && disqualified.length) {
          schedule.scheduleJob(`sendMail-top500Losers`, new Date(Date.now() + 2000), async () => {
            for await (const contestant of disqualified) {
              const sendEmailArgs = {
                templateName: 'top500Losers',
                templateData: { name: contestant.full_name.split(' ')[0] },
                toAddress: contestant.email.toLowerCase(),
              ...
              sendEmail(sendEmailArgs).then(
                () =>
                  console.log(
                    `Disqualified from Top-500 mail successfully sent to: ${contestant.email.toLowerCase()}.`
                  ),
                (err) => {
                  console.log(
                    `Error while sending disqualified from top-500 mail to: ${contestant.email.toLowerCase()}.`
                  );
                  console.dir(err.message);
                }
              );
    ...
    addProfileLike: async (parent, { customer_id, ip_address }, { models }) => {
